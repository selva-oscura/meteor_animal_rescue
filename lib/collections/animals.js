...
// 	animalId: animalId,
// }
// var animalAddressId = AnimalAddresses.insert(animaladdress);
// console.log(animalAddressId);
			return {
				_id: animalId
			};
		}else{
...
// var animalAddressId = AnimalAddresses.insert(animaladdress);
	...
	animalUpdate: function(targetAnimalId, animalAttributes){
		var now = new Date().getTime();
		var user = Meteor.user();
		// ensure user is logged in
		if(!user){
...
// var animalAddressId = AnimalAddresses.findOne({"animalId":targetAnimalId})._id;
// var removedAddress = AnimalAddresses.remove({_id:animalAddressId});
